...
// DS1302:  CE pin    -> Arduino Digital 27 22
//           I/O pin   -> Arduino Digital 29 24

//          SCLK pin  -> Arduino Digital 31 26
...
// PIN Number
#define PINNUMBER "1576"//"1576"  //"2913"
// initialize the library instance
GSM gsmAccess = false; // include a 'true' parameter f|| debug enabled
GSM_SMS sms;
...
void setup() {
  ...
  //Serial.println("clear sms setup");
...
void loop() {
  ...
  if (Message == "Ondul on") {
    ...
    Message = "";
    StatusOndul = 1;
    //Serial.println("Ondul on");
  }
  if (Message == "Ondul off") {
    digitalWrite(LED13, LOW);
    Message = "";
    StatusOndul = 0;
    //Serial.println("Ondul off");
  }
  //  }
  //-----------------------------------------------------------------------------------------------gsm
  ...
  if (N7[10] == 0 && B3[312] && (F8[199] > F10[68]) ) {
    F8[199] = F10[68];
  }
  if (N7[10] == 0 && !B3[215]) {
    F8[209] = 0;
  }
  if (N7[10] == 0 && B3[311] && (F8[209] > F10[67]) ) {
    F8[209] = F10[67];
  }
  ...
  if ((!B3[85] && !B3[86] || B3[304]) && !B3[87] && !B3[2]) {
    B3[147] = 1.0;
  }
  else {
    B3[147] = 0.0;
...
String SMS_Receive()
{
  ...
  if (sms.available())
  {
    ...
    // Delete message from modem mem||y
    sms.flush();
...
void Ecriture() {
  ...
  while (Serial3.available() > 0) { // si un caractère en réception
    ...
    if (octetReception == 10) { // si Octet reçu est le saut de ligne
      ...
      //PWM3
      //      if (chaineReception == "PWM3=ON") {
      //        //digitalWrite(LED3, HIGH);
      //
      //
      //      }
      //      if (chaineReception == "PWM3=OFF")  {
      ...
      if (chaineReception == "SMSON")  {
        GSMChecked = true;
        //Serial.println("SMSON");
      }

      if (chaineReception == "SMSOFF")  {
        GSMChecked = false;
        //Serial.println("SMSOFF");
      }
